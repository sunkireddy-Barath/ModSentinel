...
export type ConditionOperator =
  | 'lt' | 'gt' | 'lte' | 'gte' | 'eq' | 'contains' | 'not_contains' | 'matches_regex';
...
export type ActionType =
  | 'remove' | 'approve' | 'report' | 'hold' | 'ban' | 'mute' | 'flair' | 'lock' | 'distinguish';
...
export interface UserProfile {
  ...
  modHistory: Array<{ action: string; mod: string; reason: string; timestamp: number }>;
  notes: ModNote[];
  riskProfile: {
...
export type TabId = 'queue' | 'rules' | 'notes' | 'health' | 'settings';
